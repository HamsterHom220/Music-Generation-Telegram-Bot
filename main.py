...
class Generator:
    ...
    def __init__(self, accomp_volume=30, mode_name="IONIAN", lowest_note_offset=-4, combined_output=True, out_filename="output"):
...
class EvolutionaryAlgorithm(Generator):
    ...
    def crossover(self, sorted_population:  list[tuple[int, list]]):
        """
        ...
        for j in range(self.population_size//2):
            parent1 = sorted_population[randint(self.population_size//2,self.population_size-1)][1]
            parent2 = sorted_population[randint(self.population_size // 2, self.population_size - 1)][1]
    ...
    def mutation(self, population:  list[tuple[int, list]]):
        """
    ...
    def generate_accomp(self):
        ...
        '''
        final_population = sorted(self.generate_population(), key=lambda x:x[0])
        for _ in range(self.generations):
            final_population = self.mutation(self.crossover(final_population))
            for item in final_population:
                item[0] = self.compute_adaptation(item[1])
            final_population = sorted(self.generate_population(), key=lambda x: x[0])
